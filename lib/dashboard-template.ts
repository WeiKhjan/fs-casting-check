...
export function generateDashboardHtml(data: AuditDashboardData): string {
  ...
  const warningsRows = (data.warnings || []).map((w, idx) => `
        <tr>
            <td>${idx + 1}</td>
            <td><span class="exception-type">${escapeHtml(w.type)}</span></td>
    ...
    <div class="dashboard">
        <header class="header">
            ...
            <div class="header-right">
                ...
                <span class="status" style="background: linear-gradient(135deg, #8b5cf6 0%, #3b82f6 100%); margin-top: 0.5rem; font-size: 0.7rem;">🔢 Code-Verified</span>
            </div>
        </header>
        ...
        <section class="section">
            <div class="section-header">
                <h2><span class="icon icon-yellow">👁️</span>Human Review Required</h2>
                <span class="section-badge badge-warning">${(data.warnings || []).length} Items</span>
            </div>
            <div class="section-body">
                <p style="color: var(--text-secondary); margin-bottom: 1rem; font-size: 0.9rem;">
                    The following items were flagged during extraction as potentially ambiguous or uncertain.
                    Please verify these values manually against the source document.
                </p>
                <table>
                    <thead>
                        <tr>
        ...
        <footer class="footer">
            <p>Audit Casting Tool v3.0 | Extraction: Gemini AI | Verification: Deterministic Code (100% Accurate)</p>
            <p style="margin-top: 0.5rem; font-size: 0.75rem; opacity: 0.7;">All arithmetic verified by code - no LLM calculations</p>
        </footer>
    </div>
